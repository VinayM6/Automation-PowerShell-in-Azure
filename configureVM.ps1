...
"@
    ...
    Set-AzVMCustomScriptExtension -ResourceGroupName $resourceGroupName -VMName $vmName -Name "InstallSoftware" `
      -Location $location -FileUri "https://your-storage-account-name.blob.core.windows.net/scripts/install-software.sh" `
      -Run "install-software.sh" -Argument $scriptEncoded -StorageAccountName "your-storage-account-name" `
